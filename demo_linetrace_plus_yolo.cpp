// l to line trace with pi cam and y to do yolo

// ALSO space bar to stop line trace and g to go again
...
#include <opencv2/core/ocl.hpp>			// maybe this is what generates the warnings sometimes???
#include <fstream>
...
// PID stuff
int target_speed = 20, motor_change = 0, pid_error = 0;//, total_error = 0, previous_error = 0;
int targetX = 160;
...
// 	when both greens are detected, the second one is always out of bounds,
// 	so it needs to be checked the 2nd time around not the first
int RECHECK = 0;
...
cv::dnn::Net net;
void init_yolo(){
	net = cv::dnn::readNet("/home/pi/Jeremy/yolo/model100_NO_pretrain.onnx");
...
void yolo(Mat &og_img){
	
	Mat img = og_img.clone();
	flip(img, img, -1);
	...
	// not sure when these would be useful
...
int green_square(Mat &img){
	...
	Scalar low_range(70, 107, 58), high_range(100, 255, 170);
	...
	for (auto& cnt : contours) {
		...
		if(contourArea(cnt) > 1500){
			...
			// ONLY CHECK THIS IF NO GREEN HAS BEEN DETECED SO FAR.... the second green is always out of bounds
			if(RECHECK == 0){
...
void line_trace(Mat &img){
	...
	if(green_result){
		...
		*/
		...
		// go back to the beginning to get a new image and stuff??
		//continue;
	...
	* */
	...
	motor_change = (int)(pid_error*kp);// + (int)(total_error*ki) + (int)((pid_error - previous_error) * kd);
...
void main_driver(){
	...
	// default to line tracing first... cuz room 1 is always first
	char program_selection = 'L';
	...
	do{
		...
		if(program_selection == 'L'){
			printf("Line tracing...\n");
			if(!cameras[ROOM1CAM].getVideoFrame(img,99999999)){
				printf("\n\n--------- room 1 CAMERA TIMEOUT....... ---------\n\n");
			}
			printf("\ngot the image???\n");
			
			DEBUG_IMSHOW("basic line", img);
		...
		else if(program_selection == 'Y'){
			printf("Evac YOLO...\n");
			if(!cameras[ROOM2CAM].getVideoFrame(img,99999999)){
				printf("\n\n--------- room 2 CAMERA TIMEOUT....... ---------\n\n");
			}
			printf("\ngot the image???\n");
			
			DEBUG_IMSHOW("basic evac yolo", img);
	...
	cameras[ROOM1CAM].stopVideo();
	cameras[ROOM2CAM].stopVideo();
...
int main(){
	...
	// load and setup the yolo model for later
	init_yolo();
